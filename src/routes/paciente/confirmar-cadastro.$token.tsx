...
export const Route = createFileRoute("/paciente/confirmar-cadastro/$token")({
  head: () => ({
    meta: [
  ...
  component: ConfirmarCadastroPacientePage,
});

function ConfirmarCadastroPacientePage() {
  ...
  useEffect(() => {
    ...
    rodou.current = true;
    confirmPatientEmailVerification({ data: { token } })
      .then((r) => {
        if (r.ok) {
          setNome(r.nome);
          setEstado("ok");
  ...
  const reenviar = async () => {
    ...
    setReenviando(true);
    try {
      await resendPatientVerificationEmail({ data: { email, origin: window.location.origin } });
