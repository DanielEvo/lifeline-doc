...
export const Route = createFileRoute("/paciente/redefinir-senha/$token")({
  head: () => ({
    meta: [
  ...
  component: RedefinirSenhaPacientePage,
});

function RedefinirSenhaPacientePage() {
  ...
  const submit = async (e: FormEvent) => {
    ...
    setBusy(true);
    try {
      const r = await resetPatientPassword({ data: { token, novaSenha: senha } });
      if (r.ok) {
