...
export const Route = createFileRoute("/paciente/redefinir-senha")({
  validateSearch: (search: Record<string, unknown>) => ({
    token: typeof search.token === "string" ? search.token : "",
  }),
  head: () => ({ meta: [{ title: "LifeLine · Redefinir senha" }] }),
  component: PatientRedefinirSenhaPage,
});

function PatientRedefinirSenhaPage() {
  ...
  const submit = async (e: FormEvent) => {
    ...
    setBusy(true);
    try {
      const r = await resetPatientPassword({ data: { token, newPassword: pass } });
      if (r.ok) {
